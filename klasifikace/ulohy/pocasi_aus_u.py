...
# Tento soubor byl automaticky vygenerován pomocí predictor_generator.py a poté byl upraven.
...
def load_(file):
    model = load(file)
    if model["sklearn_ver"] != sklearn_ver and not ignorovat_jinou_verzi:
        return None
    return model["model"]
def dump_(model, file):
    dump({"model": model, "sklearn_ver": sklearn_ver}, file)
...
if exists(MODEL_FILE) and not pretrenovat:
    model = load_(MODEL_FILE)
    pretrenovat = model == None
    if pretrenovat:
        MODEL_FILE = MODEL_ALT_FILE
else:
    pretrenovat = True
