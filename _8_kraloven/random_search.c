...
#define VYCHOZI_HRANA_SACHOVNICE 10
...
static inline signed char random_search_thread(signed long *queens, unsigned long *pocet_prozk_stavu, signed long start, signed long stop) {
    ...
    unsigned long *pocet_prozk_stavu_arr = malloc(RANDOM_SEARCH_KOLIKRAT * sizeof(unsigned long));
    if (!pocet_prozk_stavu_arr) return 1;

    unsigned long *pocet_prozk_stavu_ = pocet_prozk_stavu_arr;
...
int main(int argc, char **argv) {
    ...
    printf("Poznámka: Náhodné procházení zopakujeme %dkrát a pak zprůměrujeme počet prozkoumaých stavů.\n", RANDOM_SEARCH_KOLIKRAT);
    alg_info(random_search, queens, "4) Náhodné hledání");
