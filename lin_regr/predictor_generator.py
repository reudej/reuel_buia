...
print("Probíhá trénování lineárně regresního modelu...", end="")
stdout.flush()
...
lin_regr_prum_pred_cas = (end-start) / test_size

print(" Test dokončen. Průměrný predikční čas:", lin_regr_prum_pred_cas, "s")

lin_regr_mae = mean_absolute_error(y_test, lin_regr_y_pred)
lin_regr_rmse = root_mean_squared_error(y_test, lin_regr_y_pred)
print("MAE:", lin_regr_mae)
print("RMSE:", lin_regr_rmse)
print()
...
while True:
    ...
    for rand_forest, pocet_stromu in zip(rand_forests, (min_pocet_stromu, max_pocet_stromu, prum_pocet_stromu)):
        if not rand_forest["pripraveno"] :
            rand_forest["model"] = RandomForestRegressor(n_estimators=pocet_stromu)
    ...
    for rand_forest in rand_forests:
        ...
        start = time.perf_counter()
        rand_forest["model"].fit(X_test, y_test)
        end = time.perf_counter()
        rand_forest["trenovaci_cas"] = end-start

        start = time.perf_counter()
        rand_forest["y_pred"] = rand_forest["model"].predict(X_test)
        end = time.perf_counter()
        rand_forest["prum_pred_cas"] = (end-start) / test_size

        rand_forest["pripraveno"] = True

        rand_forest["mae"] = mean_absolute_error(y_test, rand_forest["y_pred"])
        rand_forest["rmse"] = root_mean_squared_error(y_test, rand_forest["y_pred"])

    rmse_min, rmse_max, rmse_avg = [rand_forest["rmse"] for rand_forest in rand_forests]

    if abs(rmse_min-rmse_max) <= zanadbatelny_rozdil_rmse:
        rand_forest = rand_forest_min
        pocet_stromu = min_pocet_stromu
        break
    if rmse_avg == -1:
        ...
        break
    if abs(rmse_avg-rmse_max) <= zanadbatelny_rozdil_rmse:
...
print("MAE:", rand_forest["mae"])
print("RMSE:", rand_forest["rmse"])
print()

modely = (("LinearRegression()", "from sklearn.linear_model import LinearRegression"),
          (f"RandomForestRegressor(n_estimators={pocet_stromu})", "from sklearn.ensemble import RandomForestRegressor"))
if rand_forest["rmse"] < lin_regr_rmse:
    print("Bude použit náhodný les.")
    model = 1
else:
    print("V tomto případě bude lepší použít lineární regresi.")
    model = 0
